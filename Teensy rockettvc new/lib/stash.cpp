...
int ledblu = 2;    // LED connected to digital
  pin 9
...
void accel_degrees () {
  ...
  //converting
  angular acceleration to degrees
  gyroAngleX +=  (((GyroZ + prevgyroX) / 2)*
  dt); // deg/s * s = deg
  gyroAngleY +=  (((GyroY + prevgyroY) / 2)* dt);

  
  //complimentary filter
  double OrientationX = 0.94 * gyroAngleX + 0.06
  * accAngleX;
...
pwmX
  = ((PIDX * servo_gear_ratio) + servoX_offset);
